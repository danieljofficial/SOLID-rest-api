...
describe("Product API", () => {
  ...
  let viewerUser: Awaited<ReturnType<typeof registerTestUser>>;
  let adminRequest: ReturnType<typeof getAuthenticatedRequest>;
  let viewerRequest: ReturnType<typeof getAuthenticatedRequest>;
  ...
  beforeAll(async () => {
    ...
    adminUser = await registerTestUser("admin");
    viewerUser = await registerTestUser("viewer");

    adminRequest = getAuthenticatedRequest(adminUser);
    viewerRequest = getAuthenticatedRequest(viewerUser);
  });
  afterEach(async () => {
    // cleanUpTestProducts();
  ...
  describe("POST /products", () => {
    it("Should create a new product", async () => {
      const response = await adminRequest("post", "/products").send(testObject);
    ...
    it("Should reject product creation with missing required fields (400)", async () => {
      const response = await adminRequest("post", "/products").send({
        name: "test product",
        description: "test product",
      });
